registerController.controller('checkAccountController',['$scope', '$rootScope', '$location', function ($scope, $rootScope, $location) {
    /** 声明参数*/
    ...
    $scope.init = function () {
        ...
        $('.layui-nav-itemed .layui-nav-child').find('dd').eq(3).addClass('layui-this');

        $scope.thisMonth = new Date(parseInt($location.search().thisMonth)).Format("yyyy-MM-dd");
        $scope.listOrders();
    }

    /*获取订单列表*/
    $scope.listOrders = function () {
    	var url = '/business/checkAccount/' + $scope.page.pageIndex;
