...
<head>
    <meta charset=UTF-8>
    <link rel="stylesheet" href="/static/css/message/chat.css?v=1.0.0">
</head>
<body>
<div class="chat-items none vue-dialog-content" >
    ...
    <div class="chat-nav">
        <div class="chat-search">
            <input type="text" placeholder="搜索" v-model = "search"><%-- v-model = "search" --%>
        </div>
        <ul style="height: 566px; overflow: hidden; overflow-y: auto;">
            <template v-for = "(c, index) in chatLog" v-if = "c.length > 0">
                <li v-if = "inSearchMemberList(c)" class="chat-member-list cursor-pointer" :user-code = "getUserCode(c, false)" @click = "changeCurrent($event)">
                    <div class="fl chat-img">
                        <img :src = "getHead(c, false)" alt="">
                    </div>
                    <div class="fl chat-tip">
                        <p>{{getUserName(c, false) | subStr(20, true)}}</p>
            ...
            <div class="chat-top chat-friend" v-if = "c[c.length - 1].type == 'chat'">
                        ...
                        <template v-for = "(log, index) in c" v-if = "!log.speak0">
                            ...
                            <li class="chat-yourself" v-if = "log.mine"><%--己方消息--%>
                                ...
                                <div class="chat-text" v-if = "log.contentType != 'file'">
                                    <img v-if = "log.isSuccess == 'loading'" class="loading" src = "/static/images/loading.gif">
                                    <img v-if = "log.isSuccess == 'false'" class="loading" src = "/static/images/chat-fail.png">
                                    ...
                                    <div class="file-content">
                                        <div class="file-container">
                                            <div class="file-top">
                                                ...
                                                <div>
                                                    <img src="/static/images/chat-file.png" alt="" width="46" height="46">
                                                </div>
                                            </div>
                ...
                <div class="chat-footer">
                    ...
                    <div class="enter-send">
                        <a href="javascript:void(0)" @click = "sendMessage('txt')">发送</a>
                    </div>
                </div>
            </div>
            <div class="chat-top chat-group "  v-else>
                <div class="chat-title">
                    <div class="chat-other">
                        <%--<img :src = "getHead(c)" alt="">--%>
                        <span>{{getUserName(c)}}
                            <img src = "/static/images/arrows.png" class="show-member" @click = "showGroupMember(c, event)">
                        </span>
            ...
            <ul class="chat-member-lists">
                <li v-for = "i in lessonMember">
                    <a href="#">
                        ...
                        <img v-else src="/static/images/template-head.png" alt="">
                        <p>{{i.userName}}</p>
