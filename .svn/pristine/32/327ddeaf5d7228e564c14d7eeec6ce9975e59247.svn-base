...
<link rel="stylesheet" href="/static/css/vipcard/vip-card.css?v=1.0.1">
<div id="createCard" ng-init="init()">
    <div class="titleShow">
        <div class="createTop">
            ...
            <p class="batchCreate" ng-click="popupApplyVipCardDialog()">批量创建卡号</p>
        </div>
        <div class="cardTop clearfix" style="float: right; margin-top: -42px;">
            <button style="margin-right: 30px; cursor: pointer" ng-click="search()">搜索</button>
            <input type="text" id="acquire" placeholder="请输入服务卡号/渠道商名称" value ng-model="conditions.keyword">
        </div>
    </div>
    ...
    <div id="serviceCardPopup" style="display: none;">
        ...
        <ol class="clearfix">
            ...
            <select style="margin-left: 45px; padding: 5px; color: #9a9a9a;"
                    <%--ng-options="u.name for u in businessUsers"--%> ng-model="selectBusinessId">
                <option value="0">-选择渠道商-</option>
                <option ng-repeat="u in businessUsers" ng-value="u.id">{{u.name}}</option>
            </select>
        </ol>
