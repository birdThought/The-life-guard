layui.define(['laytpl', 'form'], function(exports) {
    ...
    Picker.fn.render = function() {
        ...
        var config = that.config,
            ...
            getAreaNameByCode = function(code, type){
            	var name = undefined;
            	for (var i = 0; i < DATA.length; i++) {
            		var d = DATA[i];
            		if(d.type == type && d.code == code){
            			name = d.name;
            			break;
            		}
            	}
            ...
            temp = function(filterName, tipInfo) {
                ...
                html += '<option id="{{item.name}}" value="{{ item.code }}" selected>{{ item.name }}</option>';
                html += '{{# }else{ }}'
                html += '<option id="{{item.name}}" value="{{ item.code }}">{{ item.name }}</option>';
                html += '{{# }; }}';
                html += '{{#  }); }}';
                html += '</select>';
                html += '</div>';
            ...
            renderData = function(type, $picker, parentCode, selectCode, init) {
                ...
                layui.laytpl(tempHtml).render(getDatas(type, parentCode, selectCode), function(html) {
                    ...
                    form.on('select(' + filter + ')', function(data) {
                        var provinceData = $('#area_show .layui-input-inline:nth-of-type(1) input').val();
//                        console.log(data)
                        var cityData = $('#area_show .layui-input-inline:nth-of-type(2) input').val();
                        var areaData = $('#area_show .layui-input-inline:nth-of-type(3) input').val();
                        var area_data_f = $('#area_data_hide li:nth-child(1)');
                        var area_data_s = $('#area_data_hide li:nth-child(2)')
                        var area_data_t = $('#area_data_hide li:nth-child(3)')
                        provinceData?area_data_f.text(provinceData):'';
                        cityData?area_data_s.text(cityData):'';
                        areaData?area_data_t.text(areaData)&area_data_t.attr('code',data.value):'';
        ...
        if (config.codeConfig) {
            ...
            for(var i = 0; i < arrPath.length; i++){
            	var name = getAreaNameByCode(arrPath[i], i + 1);
            	arrName.push(name);
            }
            $('.province-item b').text(arrName[0]);
            $('#area_data_hide li:nth-child(1)').text(arrName[0]);
            $('.city-item b').text(arrName[1]);
            $('#area_data_hide li:nth-child(2)').text(arrName[1]);
            $('.area-item b').text(arrName[2]);
            $('#area_data_hide li:nth-child(3)').text(arrName[2]);
            $('.area-item b').attr('code', arrPath[2]);
            $('#area_data_hide li:nth-child(3)').attr('code', arrPath[2]);
            switch (pType) {
                case pickerType.province:
                    //渲染省
