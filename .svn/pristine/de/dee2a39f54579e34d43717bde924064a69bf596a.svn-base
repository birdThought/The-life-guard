...
//      realLoca 等于 DW 是实时定位、等于 SOS 是SOS定位信息，等于DS是定时上传定位的内容
        ...
        TUser user = commonTrans.getEntity(TUser.class, entity.getUserId());
        //转换坐标系
        Map resultMap = BaiduMapAPI.gpsFormatBaidu(String.valueOf(lng), String.valueOf(lat));
        ...
        if((realLoca.equalsIgnoreCase("SOS") || realLoca.equalsIgnoreCase("DW")) && (cacheService.getCacheValue(CacheType.LOCATION, user.getId() + "currentLocation") == null)){
     ...
     *
     *  @param userDeviceId
     *  @param datetime 数据上传时间
     *  @param user 用户对象
     *  @param lat 标准的GPS纬度
     *  @param lat 标准的GPS纬度
     *  @return
     */
    private boolean outFence(int userDeviceId, Date datetime, TUser user, double lng, double lat,int locaType, String realLoca, String address){
        boolean bool = false;
     ...
     * @throws OperationException 
     */
    private boolean sendSMS(String smsNum, Integer locaType, String realLoca, String lng, String lat, String address, TUser user) throws SMSException, OperationException {
        boolean bool = false;
        ...
        if (locaType == 1) { // GPS_1,WIFI_2,SIM_3*/
            if ("DW".equalsIgnoreCase(realLoca)) {// 实时定位
                ...
                if (StringUtils.isNotBlank(smsNum)) {
                    address = address + "。" + BaseDefine.SYS_URL
                            + "/releaseControl.do?showMap&lng=" + lng + "&lat="
                            + lat;
                    smsService.send(user.getId(), vcodeTerminalType, smsNum.trim(), SMSCommand.SSDWGPS, false,
                            user.getUserName(), address);
                }
            } else if ("SOS".equalsIgnoreCase(realLoca)) {// 求助
                smsNum = contactsService.findFamilySos(user.getId());
                smsService.send(user.getId(), vcodeTerminalType, smsNum, SMSCommand.SOSGPS, false, user.getMobile(),
                        address);
            }
        } else {
            if ("DW".equalsIgnoreCase(realLoca)) {// 实时定位
                ...
                if (StringUtils.isNotBlank(smsNum)) {
                    address = address + "。" + BaseDefine.SYS_URL
                            + "/releaseControl.do?showMap&lng=" + lng + "&lat="
                            + lat;
                    smsService.send(user.getId(), vcodeTerminalType, smsNum.trim(), SMSCommand.SSDW, false,
                            user.getUserName(), address);
                }
            } else if ("SOS".equalsIgnoreCase(realLoca)) {// 求助
                smsNum = contactsService.findFamilySos(user.getId());
                //smsService.send(user.getId(), vcodeTerminalType, smsNum, SMSCommand.SOS, false, user.getMobile(), address);
                smsService.send(user.getId(), vcodeTerminalType, smsNum, SMSCommand.SOS, false, user.getRealName(), address);
            }
        }
        return bool;
    }
