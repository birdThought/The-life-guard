$(function() {
	/** 初始化 */
	selectInit();
	userMessageInit();	// 用户基本信息获取
	/** 事件绑定 */
	selectEventBind();	// 级联选择器
	addCaseEvent();	// 添加病程
	courseEditEventBind();	// 病程编辑按钮
	courseDeleteEventBind();	// 病程删除按钮
	caseSaveEventBind();	// 保存按钮
	caseBackEventBind();	// 返回按钮
	sortBtnEventBind();	// 排序按钮
	
	$('div.reportPop').on('change', '[name="files"]', function() {
		imgControl.uploadImg($(this)[0].files);
	});
	
	$('div.reportPop').on('click', 'div.delete>span', function() {
		var $obj = $(this).parent('div.delete').prev('img');
		var _index = parseInt($obj.attr("name").substring(3)) - 1;
//		var src = $obj.attr("src");
//		var img = [Img(_index, src)];
//		imgControl.deleteImgs(img);
		imgControl.deleteWithIndex(_index);
		imgControl.printImg();
	});
	
	$('div.reportPop').on('click', 'button[name="courseClear"]', function() {
		imgControl.clearImg();
	});
});

jeDate({
    dateCell:"[name='visitingDate'],[name='visitingDate_case']",
    format:"YYYY-MM-DD",
    isinitVal:true,
    initAddVal:[0],
    minDate:"1900-01-01",
    maxDate: jeDate.now(0),
    startMin:"1900-01-01",
    startMax:jeDate.now(0),
    zindex: 999,
    choosefun:function(elem, val) {
    	//val为获取到的时间值
    }
});

function userMessageInit() {
	var $table = $("table.addCase");
	var $trs = $table.find("tr");
	$.ajax({
		async: true,
		cache: false,
		type: 'GET',
		url: 'recordHealthFileControl.do?getUserMessage',
		success: function(result) {
			var userMessage = result.obj;
			$trs.eq(0).children("td:eq(1)").text(userMessage.name);
			$trs.eq(1).children("td:eq(1)").text(userMessage.sex);
			$trs.eq(2).children("td:eq(1)").text(userMessage.birthday);
			$trs.eq(3).children("td:eq(1)").text(userMessage.age+"岁");
		}
	});
}

function selectInit() {
	var $parentSelect = $("[name='parent']");
	$parentSelect.empty();
	$parentSelect.append('<option value="0">请选择</option>');
	
	$.ajax({
		async: true,
		cache: false,
		type: 'GET',
		url: "recordHealthFileControl.do?getChildDepartments&parentId=0",
		success: function(result) {
			var departments = result.obj;
			for(var i = 0; i < departments.length; i++) {
				var department = departments[i];
				$parentSelect.append("<option value='"+department.id+"'>"+department.name+"</option>");
			}
		}
	});
}

function selectEventBind() {
	$("[name='parent']").change(function() {
		var $childSelect = $("[name='child']");
		var optionValue = $(this).val();
		
		$childSelect.empty();
		$childSelect.append('<option value="0">请选择</option>');
		
		if(optionValue != 0) {
			$.ajax({
				async: true,
				cache: false,
				type: 'GET',
				url: "recordHealthFileControl.do?getChildDepartments&parentId="+optionValue,
				success: function(result) {
					var departments = result.obj;
					for(var i = 0; i < departments.length; i++) {
						var department = departments[i];
						$childSelect.append("<option value='"+department.id+"'>"+department.name+"</option>");
					}
				}
			});
		}
	});
}

var update_index = -1;

function addCaseEvent() {
	$('.add_btn span').parents('div.record_course').on('click',function(){
		layer.open({
			type: 1,
			title:['添加病程','text-align:center;font-size:16px;background:#fff;'],
			area:'500px',
			offset:'100px',
			btn:['确定','取消'],
			moveType: 1,
			scrollbar : false,
			zIndex: 888,
  			content: $('.reportPop'), //这里content是一个DOM
  			success: function() {
  				if(update_index == -1) {
  					modalDataClean();
  				} else {
  					$("div.layui-layer-title").html("修改病程");
  				}
  			},
  			cancel: function() {
  				// 点击取消或X掉模态框的时候触发的事件
  				
  			},
  			end: function() {
  				// 一定会执行的方法
  				/** 排序 */
  				courseControl.sort();
  				/** 构图 */
  				courseControl.appendToPage($('div.record>ul.timeline'));
  				imgControl.reset();
  				/** 排序存储 */
  				timelineControl.storage($('ul.info_list'));
  				/** 正常关闭需要归零courseEditNumber的值 */
  				update_index = -1;
  				/** 正向排序 */
  				var $sortBtn = $("div.sortBtn");
  				$sortBtn.removeClass("asc");
  				$sortBtn.click();
  			},
  			yes: function(index) {
  				
  				var visitingDate = $("[name='visitingDate_case']").val();
  				var courseType = parseInt($("[name='courseType']").val());
  				var remark = $("[name='remark']").val();
  				
  				if(visitingDate == "") {
  					layer.msg("病程日期不能为空");
  					$("[name='visitingDate_case']").focus();
  					return ;
  				}
  				if(calculate.strLength(remark) > 200) {
  					layer.msg("字数会不会太多了啊，已经超过200啦!");
  					return ;
  				}
  				
  				/** 封装 */
  				var course = Course(visitingDate, courseType, remark, imgControl.imgs, 0);
  				
  				if(update_index != -1) {
  					// 这里用一个tmp对象暂时保存原数据对象，避免因为新数据不符合规则而将原数据误删
  					var courseTmp = courseControl.getCourseByOrder(update_index);
  					courseControl.courses[update_index] = "";
  					if(!courseControl.cover(update_index, course)) {
  						courseControl.insert(update_index, courseTmp);
  	  					return ;
  	  				}
  				} else {
  					if(!courseControl.add(course)) {
  	  					return ;
  	  				}
  				}

  				/** 关闭 */
  				layer.close(index);
  			}
		});
	});

	function modalDataClean() {
		$("[name='visitingDate_case']").val(new Date().Format("yyyy-MM-dd"));
		$("select[name='courseType']").val(1);
		$("[name='remark']").val("");
		$("div.tmpDiv").empty();
		$("div.upload>span").text("");
	}
}

function sortBtnEventBind() {
	$("div.sortBtn").click(function() {
		var $root = $("ul.info_list");
		
		if($(this).hasClass("asc")) {
			timelineControl.sort(true, $root);
			$(this).removeClass("asc");
			$(this).find("span").addClass("trangleR");
			$(this).find("span").removeClass("trangle");
		} else {
			timelineControl.sort(false, $root);
			$(this).addClass("asc");
			$(this).find("span").removeClass("trangleR");
			$(this).find("span").addClass("trangle");
		}
	});
}

function courseEditEventBind() {
	/**
	 * 取消模态框自动重置数据
	 * 把li中的数据重新填写到模态框中
	 * fileNumber
	 * filesSrc
	 * 保存时courses需要查找是否有替换下标指示
	 */
	$("ul.timeline").on('click', 'span.courseEditSpan', function() {
		var $li = $(this).parents('li');
		
		update_index = parseInt($li.children('input[name="case_order"]').val());
		
		var course = courseControl.getCourseByOrder(update_index);
		
		$("[name='visitingDate_case']").val(course.visitingDate);
		$("[name='courseType']").val(course.courseType);
		$("[name='remark']").val(course.remark);
		
		imgControl.imgs = course.imgs.slice();
		imgControl.printImg();
		
		$(".add_btn span").parents('div.record_course').click();
	});
}

function courseDeleteEventBind() {
	/**
	 * 删除courses中相应下标的course
	 * 删除li标签
	 * 重新加载li_storage中存储的li标签
	 * 重新定义页面height
	*/
	
	$("ul.timeline").on('click', 'span.courseDeleteSpan', function() {
		
		var $this = $(this);
		
		layer.confirm("确认删除该病程？", function(index) {
			var $li = $this.parents("li");
			
			var removeIndex = parseInt($li.children('input[name="case_order"]').val());
			
			// page content
			$li.remove();
			avoidRemoveLiEmptyUl();
			reDefinedPageHeight();
			// data
			courseControl.remove(removeIndex);
			timelineControl.storage($('ul.info_list'));
			
			layer.close(index);
		});
	});
	
	/**
	 * 避免移除li的同时将ul的内容全部清空了，需要将隐藏着的div展示出来
	 */
	function avoidRemoveLiEmptyUl() {
		var $ul = $("ul.timeline");
		if($ul.children("li").length == 0) {
			$("div.record_default:hidden").show();
		}
	}
}

function caseSaveEventBind() {
	$("ul.saveOrback").on('click', 'li:eq(0) span', function() {
		var $title = $("[name='case_title']");
		var $child = $("#child");
		var $visitingDate = $("[name='visitingDate']");
		var $doctorDiagnosis = $("[name='case_doctorDiagnosis']");
		var $basicCondition = $("[name='case_basicCondition']");
		var $hospital = $("[name='case_hospital']");
		
		if(isBlank($title) || isBlank($hospital) || isBlank($visitingDate) || isBlank($doctorDiagnosis) || isBlank($basicCondition)) {
			return ;
		}
		if($child.val() == 0) {
			layer.msg("请选择科室");
			$child.focus();
			return ;
		}
		if($title.val().length > 30) {
			layer.msg("病历标题长度不能超过30个字符");
			$title.focus();
			return ;
		}
		if ($hospital.val().length > 30) {
			layer.msg("医院名称不能超过30个字符");
			$hospital.focus();
			return ;
		}
		if($doctorDiagnosis.val().length > 30) {
			layer.msg("医生诊断长度不能超过30个字符");
			$doctorDiagnosis.focus();
			return ;
		}
		if($basicCondition.val().length > 200) {
			layer.msg("基础病情内容长度不能超过200个字符");
			$basicCondition.focus();
			return ;
		}
		
		var data = {};
		
		var medical = {};
		
		medical.title = $title.val();
		medical.visitingDate = $visitingDate.val();
		medical.departmentId = $child.val();
		medical.doctorDiagnosis = $doctorDiagnosis.val();
		medical.basicCondition = $basicCondition.val();
		medical.hospital = $hospital.val();
		
		data.medical = medical;
		
		var courseList = new Array();
		
		var courses = courseControl.courses;
		for(var i = 0; i < courses.length; i++) {
			if(courses[i] == undefined) {
				continue;
			}
			var courseType = courseControl.getCourseTypeNameByNumber(courses[i].courseType);
			var courseMap = {"courseType":courseType,"remark":courses[i].remark,"visitingDate":courses[i].visitingDate,"img1":"","img2":"","img3":""};
			var imgs = courses[i].imgs;
			for(var j = 0; j < imgs.length; j++) {
				
				if("img"+(j+1) == "img1") {
					courseMap.img1 = imgs[j];
				}
				if("img"+(j+1) == "img2") {
					courseMap.img2 = imgs[j];
				}
				if("img"+(j+1) == "img3") {
					courseMap.img3 = imgs[j];
				}
			}
			courseList.push(courseMap);
		}
		data.courses = courseList;
		
		dataSubmit(data);
	});
	
	function isBlank($element) {
		if($element.val() == "") {
			layer.msg("请把内容填写完整");
			$element.focus();
			return true;
		}
		return false;
	}
	
	function dataSubmit(data) {
		$.ajax({
			async: true,
			cache: false,
			contentType: 'application/json; charset=utf-8',
			data: JSON.stringify(data),
			url: 'recordHealthFileControl.do?saveCase',
			beforeSend: function() {
				layer.load();
			},
			success: function(result) {
				layer.msg(result.msg);
				if(result.success) {
					setTimeout(function() {
						window.location.href = 'recordHealthFileControl.do?enter';
					}, 800);
				}
			},
			complete: function() {
				layer.closeAll('loading');
			}
		});
	}
	
}

function caseBackEventBind() {
	$("ul.saveOrback").on('click', 'li:eq(1) span', function() {
		window.location.href = "recordHealthFileControl.do?enter";
	});
}