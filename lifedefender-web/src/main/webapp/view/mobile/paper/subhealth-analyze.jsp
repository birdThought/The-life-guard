		...
		<div class="container">
			...
			<div class="item-content-border">
				<div class="comment">
					...
					<p class="text-left" v-cloak>{{basicDes}}</p>
				</div>
				<div class="comment-content" v-if = "suggestion != null" v-cloak>
					<h4 style="color:#fe933d;">健康小贴士</h4>
					<ul v-for = "(s, index) in suggestion" v-cloak>
						<li>
							<template v-if = "suggestion.length > 1">{{index + 1}}.</template>{{s}}
						</li>
					</ul>
				</div>
				<p v-cloak class="text-center"><button class="btn" @click = "again">重新评估</button></pv-cloak>
			</div>
		</div>
	</section>
	<%--<%@include file="/context/mainFooter_1.jsp"%>--%>
	<script src="/static/plugins/bootstrap/js/bootstrap.min.js"></script>
	<script src="/static/mobile/paper/js/subhealth.js"></script>
	<script>
		health.vm.results = '${subHealthStandard}' == '[]' ? '' : JSON.parse('${subHealthStandard}');
        health.vm.score = '${score}';
