...
;(function(root, factory) {
    ...
    jedfn.createDaysHtml = function(ys, ms, opts, boxCell){
        ...
        //判断禁用启用是长度，并设置成正则  done 重写规则 217-08-04
        if(ends.length > 0){
            ...
            dayreg = dayreg == '' ? 'none' : dayreg;
            // console.log('dayreg:', dayreg)
            // var dayreg = new RegExp(ends[0].replace(/,/g,"|")); //[1,2,3]
            dayreg = new RegExp(dayreg);
        }
        //上一月剩余天数
        for (var p = prevDaysNum - firstWeek + 1; p <= prevDaysNum; p++, count++) {
            ...
            if(ends.length > 0){
                if (dateOfLimit(prevM.y, prevM.m, p, false)){
                    ...
                    pCls = "disabled";  //其他月的日期全部做“disabled”处理
                    /*if(dayreg.test(p)){
        ...
        for(var n = 1, nlen = 42 - count; n <= nlen; n++){
            ...
            if(ends.length > 0){
                if (dateOfLimit(nextM.y, nextM.m, n, false)){
                    ...
                    nCls = "disabled";  //其他月的日期全部做“disabled”处理
                    /*if(dayreg.test(n)){
