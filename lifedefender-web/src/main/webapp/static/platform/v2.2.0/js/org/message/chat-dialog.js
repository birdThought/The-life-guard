/**
 * 消息对话框
 * Created by wenxian.cai on 2017/5/4.
 */
...
chat.vm = new Vue({
    ...
    methods: {
        ...
        isShowTimeTip: function (pre, now, next) {
            ...
            if (now.time - pre.time > 120000) {  //2分钟间隔
                return true;
            }
            return false;
        }
        ...
        search: function () {
            ...
            setTimeout(function () {
                ...
                for (var i in chat.vm.chatLog) {
                    if (chat.vm.chatLog[i][chat.vm.chatLog[i].length - 1].userName.indexOf(chat.vm.search) != -1) {
                        temp[c] = chat.vm.chatLog[i];
                    }
                }
                chat.vm.searchMember = Object.assign({}, chat.vm.chatLog, temp)
            }, 50);
        }
...
chat.listHistory = function (ownerUserCode, otherUserCode, page) {
    ...
    chat.vm.unreadCount[otherUserCode] = 0;
    ...
    chat.vm.chatLog = chat.vm.chatLog == null ? [] : chat.vm.chatLog;
    //将有未读信息的用户加入members
    for (var i in chat.vm.chatLog) {
        var bool = false;
    ...
    if (chat.vm.chatLog[otherUserCode] == null) {
        ...
        var message = {
            ...
            "userName": chat.vm.currentMember.userName,
            "isSuccess": 'true',
            "speak0": true
        }
